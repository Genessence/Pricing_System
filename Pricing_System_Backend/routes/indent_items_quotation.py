...
async def create_indent_quotation(quotation_data: IndentItemsQuotationCreate):
    """Create a new indent items quotation."""
    return indent_items_quotation_controller.create_indent_quotation(quotation_data)
...
async def get_indent_quotations(
    ...
    return indent_items_quotation_controller.get_multi(skip=skip, limit=limit)
...
async def get_quotations_by_rfq(rfq_id: UUID = Path(..., description="RFQ ID")):
    ...
    return indent_items_quotation_controller.get_quotations_by_rfq(rfq_id)
...
async def get_quotations_by_vendor(vendor_id: UUID = Path(..., description="Vendor ID")):
    ...
    return indent_items_quotation_controller.get_quotations_by_vendor(vendor_id)


@router.get("/indent-item/{indent_item_id}", response_model=List[IndentItemsQuotationListResponse])
async def get_quotations_by_indent_item(indent_item_id: UUID = Path(..., description="Indent item ID")):
    """Get quotations for a specific indent item."""
    return indent_items_quotation_controller.get_quotations_by_indent_item(indent_item_id)


@router.get("/{quotation_id}", response_model=IndentItemsQuotationResponse)
async def get_indent_quotation(quotation_id: UUID = Path(..., description="Quotation ID")):
    """Get an indent items quotation by ID."""
    return indent_items_quotation_controller.get(quotation_id)
...
async def update_indent_quotation(
    ...
    return indent_items_quotation_controller.update_quotation(quotation_id, quotation_data)


@router.delete("/{quotation_id}")
async def delete_indent_quotation(quotation_id: UUID = Path(..., description="Quotation ID")):
    """Delete an indent items quotation."""
    return indent_items_quotation_controller.delete(quotation_id)
