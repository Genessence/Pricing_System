...
const ServiceItemRow = ({ 
  ...
  const calculateAmount = (rate, quantity) => {
    ...
    console.log('calculateAmount:', { rate, quantity, rateValue, qtyValue, result });
    return result;
  };
  ...
  return (
    <tr className="border-b border-border hover:bg-muted/30 transition-colors h-16">
      ...
      <td className="p-3 bg-card sticky left-148 z-10 border-r border-border">
        <input
          type="number"
          value={localValues.requiredQuantity !== undefined ? localValues.requiredQuantity : (item?.requiredQuantity || '')}
          onChange={(e) => {
          ...
          onBlur={(e) => {
            ...
            setLocalValues(prev => ({ ...prev, requiredQuantity: undefined }));
          }}
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              const value = e?.target?.value;
      ...
      <td className="p-3 bg-card sticky left-188 z-10 border-r border-border">
        <input
          type="number"
          value={localValues.rate !== undefined ? localValues.rate : (item?.rate || '')}
          onChange={(e) => {
            const value = e?.target?.value;
      ...
      {quotes?.map((quote, quoteIndex) => {
        ...
        <td key={quoteIndex} className="p-3 border-r border-border">
          <div className="space-y-2">
            ...
            <input
              type="number"
              value={localValues[`quote_${quoteIndex}`] !== undefined ? localValues[`quote_${quoteIndex}`] : (quote?.rate || '')}
              onChange={(e) => {
                const value = e?.target?.value;
                setLocalValues(prev => ({ ...prev, [`quote_${quoteIndex}`]: value }));
              }}
              onBlur={(e) => {
                const value = e?.target?.value;
                onRateChange(quoteIndex, value);
                setLocalValues(prev => ({ ...prev, [`quote_${quoteIndex}`]: undefined }));
              }}
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  const value = e?.target?.value;
